...
from cornode import TRITS_PER_TRYTE, TrytesCodec
from cornode.crypto import Curl, HASH_LENGTH
from cornode.exceptions import with_context
from cornode.json import JsonSerializable
...
class TryteString(JsonSerializable):
  ...
  def as_bytes(self, errors='strict'):
    ...
    :raise:
      - :py:class:`cornode.codecs.TrytesDecodeError` if the trytes cannot
...
class Address(TryteString):
  ...
  def __init__(self, trytes, balance=None, key_index=None):
    ...
    References:
      - :py:class:`cornode.crypto.addresses.AddressGenerator`
    """
