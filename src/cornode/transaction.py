...
from cornode import Address, Hash, Tag, TryteString, TrytesCompatible, \
...
from cornode.crypto.signing import KeyGenerator, SignatureFragmentGenerator, \
...
class Transaction(JsonSerializable):
  ...
  def __init__(
    ...
    References:
      - :py:meth:`cornode.api.Strictcornode.get_inclusion_states`
      - :py:meth:`cornode.api.cornode.get_transfers`
    """
...
class ProposedTransaction(Transaction):
  ...
  Provide to :py:meth:`cornode.api.cornode.send_transfer` to attach to
...
class Bundle(JsonSerializable, Sequence[Transaction]):
  ...
  citizens in cornode; only transactions get stored in the Tangle.

  Instead, Bundles must be inferred by following linked transactions
  with the same bundle hash.

  References:
    - :py:class:`cornode.commands.extended.get_bundles.GetBundlesCommand`
  """
  @classmethod
